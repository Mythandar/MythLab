...
Normal data is stored under %LOCALAPPDATA%\Homelab.RemoteManager. No credentials are collected in this milestone. Change the display name in build/Identity.props. See [architecture](docs/ARCHITECTURE.md), [roadmap](docs/ROADMAP.md) and [dependencies](docs/THIRD-PARTY-NOTICES.md).

No GitHub remote or installer has been created.

Verification: Release build clean, 35 tests passing, offscreen WPF smoke checks passing. See [verification details](docs/VERIFICATION.md). Run the UI harness with `dotnet run --project tools/RemoteManager.SmokeTests -c Release`.
