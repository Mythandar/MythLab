...
| SSH known-host identities and trust decisions | Data/ssh/known-hosts.json (reserved; Milestone D) |
| Logs | Data/logs |
| Embedded terminal WebView2 user data/cache | Data/WebView2 (reserved; created only when a terminal needs it) |
...
## Milestone D implementation gates
...
The current milestone reserves paths and checks general data/log write access but intentionally does not add the WebView2 SDK or pretend to detect a renderer that has not been adopted.
